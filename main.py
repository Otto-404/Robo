...
ACTION_MAP = {
    str(Color.RED): STOP,
    str(Color.GREEN): FORWARD_FAST,
    str(Color.BLUE): TURN_RIGHT,
    str(Color.YELLOW): TURN_LEFT,
    str(Color.WHITE): BACKWARD_FAST,
}
...
def update_action():
    ...
    while True:
        ...
        color_code = str(detected_color) if detected_color is not None else None
        new_action = ACTION_MAP.get(str(detected_color))
        ...
        if ultrasonic_sensor.distance() < 250:
            ...
            target_angle = arm_motor.angle() + 30
            arm_motor.run_angle(ARM_MOTOR_SPEED, 30, wait=False)
            while abs(arm_motor.angle() - target_angle) > 2:
                yield

            target_angle = arm_motor.angle() - 60
